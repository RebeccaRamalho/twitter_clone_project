...
exports.newAccount = (request, response) => {
  ...
  user.getByUsername(
    username /*what i send to the model*/,
    (error, result) /*what i get from the model*/ => {
      ...
      /*b_User error managment (i check if username already exist in db)*/
        console.log(
          "you have great tast! A username with this pseudo already exist sorry :("
        );
      } else {
        /*2-Password hashing*/
...
exports.authenticate = (request, response) => {
  ...
  /*2_*/
  user.getByUsername(username, async (
    /*what i send to the model*/ error,
    result
    ...
    /*b_User error managment (i check if username exist in db)*/
      await request.flash(
        "warning",
        "We dont remember you! Are you sure this is your username?"
      );
