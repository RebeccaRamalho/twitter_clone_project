...
exports.create = (user, callback) => {
  db.query(`INSERT INTO users(last_name, first_name,  bith_date, username, mail, password, phone, city) value ("${user.last_name}", "${user.first_name}", "${user.birthday}", "${user.username}", "${user.mail}", "${user.password}","${user.phone}", "${user.city}");`, (error, result) => {
    if (error) {
      console.log("error: ", error);
      callback(error, null);
      return;
